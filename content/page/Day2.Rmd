...
We can download the attached Excel file [HERE](https://github.com/tidy-MN/R-camp-penguins/raw/main/content/data/crew_food_prefs.xlsx)
...
# <i class="fa fa-video-camera"></i> <span style="font-size: 90%"> [Forks in the road: `if_else` + `case_when`](videos/day2/02-2-fork-in-the-road)
...
```{r, eval=F}
case_when(days_on_trip <= 7 ~ "Short trip",
          days_on_trip <= 14 ~ "Long trip",
...
crew_df <- crew_df %>%
           mutate(region = if_else(country %in% c("United States America", "Canada", "Mexico"),
                                   "North America",
                                   "Other"))
```
...
# Use mutate to add a column
crew_df <- crew_df %>%
           mutate(cocoa_check = if_else(favorite_goody %in% c("brownies", "chocolate croissant"),
                                        "Needs cocoa",
                                        "No cocoa")
```
...
# <i class="fa fa-video-camera"></i> <span style="font-size: 90%"> Simmer it down: Summarize the data
...
```{r, eval=F}
# How many Canadiens?
summarize(crew_df, canadiens = sum(country == "Canada"))

# Mean days on board ship?
summarize(crew_df, days_avg = mean(days_on_ship))

# How many Long trippers
summarize(crew_df, n_long_trippers = sum(time_onboard == "Long trip"))


# Total meals
crew_df <- mutate(df, total_meals = meals_per_day * days_on_ship)

summarize(crew_df, total_meals = sum(total_meals))
```
...
crew_df %>% group_by(country) summarize(n_crew = n())

# Total meals per goody type
crew_df %>% group_by(favorite_goody) %>% summarize(total_meals = sum(total_meals))
```
