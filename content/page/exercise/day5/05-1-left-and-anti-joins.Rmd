...
```{r}
penguin_names <- read_csv("penguin_names.csv")
```
...
Fantastic, looks like the column names all match, but if you *reallllly* want to be sure you can always use the function `intersect` to see where the names line up!

```{r}
intersect(names(penguins_raw), names(penguin_names))
```

Now it's time to join up! Join functions are smart, so if your columns have the same names it will automatically figure out which ones to join on so you don't have to specify but if they don't all you have to do is pass the function an argument called `by` where you give it specific information on what to join by. 
