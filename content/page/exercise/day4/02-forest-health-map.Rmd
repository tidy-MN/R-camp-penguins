...
If you want to save the shapefile on your computer for the longterm, you can use the [st_write function](https://r-spatial.github.io/sf/reference/st_write.html).

<br>

```{r, read-forests, cache=F}
library(tidyverse)
library(sf)
...
![Split polygons across two counties from GIS stackexchange.com](https://i.stack.imgur.com/vLlAZ.jpg){width=55%}
...
Now we need to join the summed forest damage data to the county spatial data so that we have the information we need to present the data as a map. We use `left_join` below to add the forest damage totals to the counties polygon data based on the `NAME` of the county. We'll cover joins in great detail next session, but this is a good preview. 
...
```{r exer-tabs34, results='asis', echo=F}
source("../../insert_tabs.R")
