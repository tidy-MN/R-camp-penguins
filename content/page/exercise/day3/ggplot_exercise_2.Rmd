---
title: "ggplot2: the remix - Exercise 2"
output: 
  blogdown::html_page: 
    highlight: tango
...
Make sure you are still in your project that you created for doing exercises and then make a new R script. Save it as soon as you make it and give it a good name like `exercise_2_day_4.R` or `herriot.R` and you'll be ready to go!
...
So make it work, we had to do three things when using the `geom_text` (or `geom_label` - try both and see which one you like the best!)
...
ggplot(penguins_raw_clean %>% 
  ...
  geom_point()+
  geom_errorbar(width = 0.5, linewidth = 1.25,
