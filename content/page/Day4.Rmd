...
To transform from one projection to another we use `st_transform()`, and assign the new projection with the argument `crs = 4269`.

```{r}
trees_in_san_fran <- st_transform(trees_in_san_fran, crs = 4269)
```
...
We may also add an kilometer column by dividing the calculated distance by 1,000 meters. 
...
Now we can determine the closest tree in San Francisco to our loon by using the *min()* function, which will return the minimum calculated value in a column. 
...
> **That's our tree!** Number `256051`: the bark shedding *Platanus x hispanica*, aka, the London plane tree. And only ~2,500 kilometers away.
